...
const char *satName;
int satId;
float satElevation = -45;
float satAzimuth = 160;
...
void orientationTest(float currentElevation)
{
    ...
    Serial.print("Orientation Elevation check:  ");
    ...
    if ((currentElevation >= (0 - 10) && currentElevation <= (0 + 10)) && (satElevation <= minElevation)) 
    {
    ...
    if (currentAzimuth > (satAzimuth - 15) && currentAzimuth < (satAzimuth + 15))
    {
        panningLeft = false;
        panningRight = false;
    }
...
void loop() 
{
    ...
    if (mpu.update()) {
        ...
        if (millis() > prev_ms + 100) {
            ...
            orientationTest(currentElevation);
