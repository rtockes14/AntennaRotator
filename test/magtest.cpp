...
void orientationTest()
{
    ...
    if ((currentElevation >= (0 - 3) && currentElevation <= (0 + 3)) && (satElevation <= 0)) 
    {
    ...
    if ((panningLeft) && !(currentAzimuth < satAzimuth) && alreadyPanning == false)
    {
        digitalWrite(INPUT_1, HIGH);
        digitalWrite(INPUT_2, LOW);
    ...
    if ((panningRight) && !(currentAzimuth > satAzimuth) && alreadyPanning == false)
    {
        digitalWrite(INPUT_1, LOW);
        digitalWrite(INPUT_2, HIGH);
        analogWrite(ENABLE_A, 55); // pan
